...
class HrAttendanceLeave(models.Model):
    ...
    _rec_name = "day_type"
    _order = "work_day desc, user_name asc"
    ...
    user_name = fields.Char(
        string="User Name", related="employee_id.user_id.name", store=True, copy=False
    )
    display_name = fields.Char(
        compute="_compute_attendance_leave_info", store=True, copy=False
    )
    ...
    def _compute_attendance_leave_info(self):
        for line in self:
            day_type = _("Normal Day")
            if line.calendar_leave_id:
                day_type = line.calendar_leave_id.name
            if line.leave_type_id:
                day_type = line.leave_type_id.name
            non_remunerated_hours = 0
            extra_hours = 0
            if not line.leave_type_id and not line.calendar_leave_id:
                extra_hours = line.worked_hours - line.hours_to_work
            if line.calendar_leave_id or (
                line.leave_type_id and line.leave_type_id.time_type == "other"
            ):
                if line.worked_hours == 0:
                    extra_hours = (line.hours_to_work - line.remunerated_hours) * -1
                else:
                    if line.worked_hours <= line.remunerated_hours:
                        extra_hours = (
                            line.hours_to_work
                            - line.remunerated_hours
                            + line.worked_hours
                        )
                    else:
                        extra_hours = line.worked_hours - (
                            line.hours_to_work - line.remunerated_hours
                        )

            if line.leave_type_id and line.leave_type_id.time_type != "other":
                non_remunerated_hours = line.hours_to_work - line.worked_hours
                if non_remunerated_hours > line.hours_to_work:
                    non_remunerated_hours = line.hours_to_work
                if line.worked_hours == 0:
                    extra_hours = line.hours_to_work * -1
                elif line.worked_hours < line.hours_to_work:
                    extra_hours = line.worked_hours - line.hours_to_work
                else:
                    extra_hours = line.hours_to_work - line.worked_hours
            line.day_type = day_type
            line.non_remunerated_hours = non_remunerated_hours
            line.extra_hours = extra_hours
            line.display_name = line.employee_id.name
